...
//        mCanvas.drawLine(200f, 200f, 400f, 100f, paint)
    ...
    fun drawMatr() {
        ...
        for (i in 0 until N) {
            ...
            for (j in 0..i) {
                drawTile(
                    (i - j) * xHightTile + matrX,
                    (i + j) * yHightTile + dMatrY + yHightTile,
                    map[i][j],
                    paint
                )
                drawTile(
    ...
    fun logicLongTapFun(cord: Cord) {
        if (cord.I > N - 1 || cord.J > N - 1 || cord.I < 0 || cord.J < 0) {
            ...
            if (map[y][x].city.name != "") {
                ...
                builder.setMessage(
                    "Name: ${map[y][x].city.name} ${map[y][x].city.type}\n" +
                            "Hp: ${map[y][x].city.hp}\n" +
        ...
        if (cord.I > N - 1 || cord.J > N - 1 || cord.I < 0 || cord.J < 0) {
            ...
            if (map[y][x].allocation == 1) {
                ...
                if (manager.attackCity(0, map[y][x].city.id) == 1) {
                    ...
                    Toast.makeText(context, "You have destroyed enemy city.", Toast.LENGTH_SHORT)
                        .show()
                    manager.clearTile(0)
