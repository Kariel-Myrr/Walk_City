...
class Manager(private val context: Context) {
    ...
    private fun initMap() {
        ...
        for (i in 0 until map.y) {
            ...
            for (e in 0 until map.x) {
                ...
                Log.d(
                    TAG,
                    "resourceTile[$i][$e]: wood = $tree stone = $stone iron = $iron food = $food fuel = $fuel people = $people"
                )
            }
        }
    }
    ...
    private fun initCity(id: Int) {
        ...
        Log.d(
            TAG,
            "city: name = ${tmpCity.name} hp = ${tmpCity.hp} type = ${tmpCity.type} active = ${tmpCity.active} people = ${tmpCity.people} damage = ${tmpCity.damage} protection = ${tmpCity.protection} idInventory = ${tmpCity.idInventory} x = ${tmpCity.x} y = ${tmpCity.y}"
        )
        city.add(tmpCity)
    }
    ...
    private fun initInventory(id: Int) {
        ...
        Log.d(
            TAG,
            "inventory: idItemProtection = ${tmpInventory.idItemProtection} idItemResource = ${tmpInventory.idItemResource} idItemWeapon = ${tmpInventory.idItemWeapon}"
        )
    ...
    private fun initResource(cityId: Int) {
        ...
        Log.d(
            TAG,
            "resource: wood = ${tmpResource.wood} stone = ${tmpResource.stone} iron = ${tmpResource.iron} food = ${tmpResource.food} fuel = ${tmpResource.fuel}"
        )
    ...
    fun init() {
        ...
        for (i in 0 until countCity) {
            ...
            Log.d(
                TAG,
                "tile[city[$i].x][city[$i].y].city.id = ${tile[city[i].y][city[i].x].city.id}"
            )
        }
        allocation()
    ...
    private fun downloadCity() {
        ...
        if (cityList.size != 0) {
            city = cityList
            for (i in 0 until countCity) Log.d(
                TAG,
                "city: name = ${city[i].name} hp = ${city[i].hp} type = ${city[i].type} active = ${city[i].active} people = ${city[i].people} damage = ${city[i].damage} protection = ${city[i].protection} idInventory = ${city[i].idInventory} x = ${city[i].x} y = ${city[i].y}"
            )
    ...
    private fun downloadCity(id: Int) {
        ...
        if (cityList.size != 0) {
            ...
            Log.d(
                TAG,
                "city: name = ${city[id].name} hp = ${city[id].hp} type = ${city[id].type} active = ${city[id].active} people = ${city[id].people} damage = ${city[id].damage} protection = ${city[id].protection} idInventory = ${city[id].idInventory} x = ${city[id].x} y = ${city[id].y}"
            )
        } else Log.e(TAG, "ERROR: cityList.size = ${cityList.size}")
    }
    ...
    private fun downloadInventory() {
        ...
        if (inventoryList.size != 0) {
            ...
            for (i in 0 until countCity) Log.d(
                TAG,
                "inventory: idItemProtection = ${inventory[i].idItemProtection} idItemResource = ${inventory[i].idItemResource} idItemWeapon = ${inventory[i].idItemWeapon}"
            )
    ...
    private fun downloadInventory(id: Int) {
        ...
        if (inventoryList.size != 0) {
            ...
            Log.d(
                TAG,
                "inventory: idItemProtection = ${inventory[id].idItemProtection} idItemResource = ${inventory[id].idItemResource} idItemWeapon = ${inventory[id].idItemWeapon}"
            )
    ...
    private fun downloadProtection() {
        ...
        if (protectionList.size != 0) {
            ...
            for (i in 0 until countCity) Log.d(
                TAG,
                "protection: slot = ${protection[i].slot}"
            )
    ...
    private fun downloadResource() {
        ...
        if (resourceList.size != 0) {
            for (i in 0 until countCity) {
                ...
                Log.d(
                    TAG,
                    "resource: wood = ${resource[i].wood} stone = ${resource[i].stone} iron = ${resource[i].iron} food = ${resource[i].food} fuel = ${resource[i].fuel} people = ${resource[i].people}"
                )
            }
        } else Log.e(TAG, "ERROR: resourceList.size = ${resourceList.size}")
    }
    ...
    private fun downloadResource(id: Int) {
        ...
        if (resourceList.size != 0) {
            ...
            Log.d(
                TAG,
                "resource: wood = ${resource[id].wood} stone = ${resource[id].stone} iron = ${resource[id].iron} food = ${resource[id].food} fuel = ${resource[id].fuel}"
            )
        } else Log.e(TAG, "ERROR: resourceList.size = ${resourceList.size}")
    }

    private fun downloadWeapon() {
        //Log.d(TAG, "downloadWeapon() test 1")
        ...
        if (weaponList.size != 0) {
            ...
            for (i in 0 until countCity) Log.d(
                TAG,
                "weapon: weapon.slots.size = ${weapon[i].slots.size} weapn.storage.size = ${weapon[i].storage.size}"
            )
    ...
    private fun downloadWeapon(id: Int) {
        ...
        if (weaponList.size != 0) {
            ...
            Log.d(
                TAG,
                "weapon: weapon.slots.size = ${weapon[id].slots.size} weapn.storage.size = ${weapon[id].storage.size}"
            )
        } else Log.e(TAG, "ERROR: weaponList.size = ${weaponList.size}")
    }
    ...
    private fun downloadMap() {
        ...
        } else Log.e(TAG, "ERROR: _map.size = ${tmpMap.size}")
    }
        ...
        if (tileList.size != 0) {
            for (i in 0 until map.y) {
                ...
                for (e in 0 until map.x) {
                    ...
                    val type = tile[i][e].type
                    val idItemResource = tile[i][e].idItemResource
                    Log.e(TAG, "tile[$i][$e]: type = $type idItemResource = $idItemResource")
                }
            }
    ...
    fun downloadResourceTile(i: Int, e: Int) {
        ...
        if (resourceTileList.size != 0) {
            ...
            Log.d(
                TAG,
                "resourceTile[$i][$e]: wood = $tree stone = $stone iron = $iron food = $food fuel = $fuel"
            )
        } else Log.e(TAG, "ERROR: resourceTileList.size = ${resourceTileList.size}")
    }
    ...
    fun download() {
        ...
        for (i in 0..5) {
            for (e in 0..5) Log.d(TAG, "tile[$i][$e].busy = ${tile[i][e].busy}")
        }
        allocation()
        ...
        when (type) {
            1 -> {
                ...
                return if (resource[cityId].wood >= 2 && weapon[cityId].storage.size < 10) {
                    ...
                    Log.d(
                        TAG,
                        "Not enough resources for type 1 or not enough space in storage"
                    )
                    0
                }
            }
            2 -> {
                return if (resource[cityId].wood >= 3 && resource[cityId].stone >= 1 && weapon[cityId].storage.size < 10) {
                    ...
                    Log.d(
                        TAG,
                        "Not enough resources for type 2 or not enough space in storage"
                    )
                    0
                }
            }
            3 -> {
                return if (resource[cityId].wood >= 1 && resource[cityId].stone >= 2 && weapon[cityId].storage.size < 10) {
                    ...
                    Log.d(
                        TAG,
                        "Not enough resources for type 3 or not enough space in storage"
                    )
                    0
                }
            }
            4 -> {
                return if (resource[cityId].wood >= 3 && resource[cityId].stone >= 3 && weapon[cityId].storage.size < 10) {
                    resource[cityId].wood -= 3
                    ...
                    Log.d(
                        TAG,
                        "Not enough resources for type 4 or not enough space in storage"
                    )
                    0
                }
            }
            5 -> {
                return if (resource[cityId].wood >= 2 && resource[cityId].stone >= 4 && weapon[cityId].storage.size < 10) {
                    resource[cityId].wood -= 2
                    ...
                    Log.d(
                        TAG,
                        "Not enough resources for type 5 or not enough space in storage"
                    )
                    0
                }
            }
            6 -> {
                return if (resource[cityId].wood >= 4 && resource[cityId].stone >= 6 && resource[cityId].iron >= 2 && weapon[cityId].storage.size < 10) {
                    ...
                    Log.d(
                        TAG,
                        "Not enough resources for type 6 or not enough space in storage"
                    )
                    0
                }
            }
    ...
    private fun changeWeapon(cityId: Int, num: Int) {
        if (num < weapon[cityId].storage.size) {
    ...
    fun attacLogic(cityId: Int): Int {
        ...
        if (city[cityId].x + 1 < map.x && stat == 1) {
            if (tile[city[cityId].y][city[cityId].x + 1].busy && (tile[city[cityId].y][city[cityId].x + 1].city.name == "my" || cityId == 0)) {
        ...
        if (city[cityId].y + 1 < map.y && stat == 1) {
            if (tile[city[cityId].y + 1][city[cityId].x].busy && (tile[city[cityId].y + 1][city[cityId].x].city.name == "my" || cityId == 0)) {
    ...
    fun changeLogic(cityId: Int) {
        ...
        for (i in 0 until weapon[cityId].storage.size) if (weapon[cityId].storage[i] > max || weapon[cityId].slots.size < 2) changeWeapon(
            cityId,
            i
        )
    }
